...
extern "C" {
#endif
/* LVGL 9.5 includes LV_DRAW_SW_ASM_CUSTOM_INCLUDE before the header that
 * completes lv_draw_sw_blend_fill_dsc_t / _image_dsc_t (they moved to the
 * private header after 9.2). The wrappers below read their fields, so pull
 * it in here via the lvgl root, which every lvgl consumer has on its path. */
