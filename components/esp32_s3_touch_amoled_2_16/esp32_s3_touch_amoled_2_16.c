...
/* Draw-buffer experiment knobs live in include/ml_draw_buf_cfg.h */
#include "ml_draw_buf_cfg.h"

/* Weak hook: the app can reserve a contiguous internal region at boot and
 * release it here, right before the adapter allocates the draw buffers. */
void __attribute__((weak)) ml_pre_draw_buf_alloc(void) {}
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    const bsp_display_config_t disp_config = {
        .max_transfer_sz = BSP_LCD_H_RES * ML_DRAW_BUF_HEIGHT * BSP_LCD_BITS_PER_PIXEL / 8,
    };
    ...
    esp_lv_adapter_display_config_t disp_cfg = {
        ...
        .profile = {
            ...
            .buffer_height = ML_DRAW_BUF_HEIGHT,
            .use_psram = ML_DRAW_BUF_USE_PSRAM,
...
/* rotation must go through the panel driver (not raw MADCTL) so its internal
 * swap_axes/mirror state matches the hardware — otherwise partial updates land
 * in the wrong place once rotated */
esp_lcd_panel_handle_t bsp_display_panel_handle(void)
{
    return panel_handle;
}

/* rotation needs to re-map the touch axes along with the panel */
...
{
    /* adapter returns esp_err_t (0 = success); the bool API is true = success */
